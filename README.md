...
### 🍏 macOS 离线包安装方法（绕过签名隔离）
如果你直接下载了编译好的 macOS `.zip` 安装包，由于本项目采用了开源方案并未强制使用每年 $99 的付费苹果开发者签名，首次打开时系统可能会提示“已损坏”或直接被安全机制拦截（Killed）。
**一键解决步骤：**
1. 解压 `.zip`，将解压出的 `我爱上班.app` 拖入系统的 `应用程序 (Applications)` 文件夹。
2. 打开 macOS自带的 `终端 (Terminal)` 应用。
3. 复制并运行以下命令，解除系统的网络下载隔离：
   ```bash
   xattr -cr /Applications/我爱上班.app
   ```
4. 解除后，直接双击运行即可！不仅能正常打开主程序，也能完美添加和同步桌面小组件！
